...
class StrategyRegistryAdapter:
    ...
    def build_register_strategy_tx(
        ...
        *,
        ...
        """
        Monta tx para StrategyRegistry.registerStrategy(...).

        Deve ser assinada por quem é owner do contrato (admin Wallet).
        """
        from_addr = Web3.to_checksum_address(admin_wallet)

        fn = self.contract.functions.registerStrategy(
            Web3.to_checksum_address(adapter),
            Web3.to_checksum_address(dex_router),
            Web3.to_checksum_address(token0),
            Web3.to_checksum_address(token1),
    ...
    def build_update_strategy_tx(
        ...
        *,
        ...
        """
        Monta tx para StrategyRegistry.updateStrategy(...).
        """
        from_addr = Web3.to_checksum_address(admin_wallet)

        fn = self.contract.functions.updateStrategy(
            int(strategy_id),
            Web3.to_checksum_address(adapter),
            Web3.to_checksum_address(dex_router),
            Web3.to_checksum_address(token0),
    ...
    def build_set_strategy_active_tx(
        ...
        *,
        ...
        """
        Monta tx para StrategyRegistry.setStrategyActive(...).
        """
        from_addr = Web3.to_checksum_address(admin_wallet)

        fn = self.contract.functions.setStrategyActive(
            int(strategy_id),
            bool(active),
        )
